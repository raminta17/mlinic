...
function theme_stylesheets(){
	...
	$styles_path = ASSETS_URL . '/main/css/style.css';

	if( $styles_path ) {
        wp_register_style('fonts', 'https://fonts.googleapis.com/css2?family=Montserrat&display=swap', array(), false, 'all');
        wp_register_style('bs', 'https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css', array(), false, 'all');
		wp_register_style('main-css', ASSETS_URL . '/main/css/style.css', array('fonts', 'bs'), false, 'all');
...
function createPostTypes() {
	...
	foreach( $themePostTypes as $postType => $postTypeSingular ) {
		...
		$myArgs = $defaultArgs;
		$slug = 'project' . '-' . sanitize_title( $postType );
